...
Node* reverseListIterative(Node* head) {
    ...
    while (current != nullptr) {
        ...
        current->next = prev; // Reverse the link: point current to the previous node
        prev = current;      // Move previous to current
        current = next;      // Move current to the stored next node
    }

    return prev; // The new head is the previous pointer (pointing to the last node)
}
...
int main() {
    ...
    // Choose either the iterative or recursive approach:
