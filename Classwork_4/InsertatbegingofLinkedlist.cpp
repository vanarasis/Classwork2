...
void push(Node** head_ref, int new_data)
{
    ...
    // 4. Update the head pointer to point to the new node, making it the new head of the list
    (*head_ref) = new_node;
}
...
int main() {
    ...
    push(&head, 4);   // Insert node with data 4
    push(&head, 9);   // Insert node with data 9
    push(&head, 7);   // Insert node with data 7
